...
class MinSpec extends Properties("Min") {
  ...
  property("Min[Int Refined Greater[1]]") = secure {
    Min[Int Refined Greater[1]].min.value ?= 2
  }

  property("Min[Int Refined Greater[0]]") = secure {
    Min[Int Refined Greater[0]].min.value ?= 1
  }

  property("Min[Long Refined Greater[0]]") = secure {
    Min[Long Refined Greater[0]].min.value ?= 1L
  }

  property("Min[Float Refined Greater[0]]") = secure {
    Min[Float Refined Greater[0f]].min.value ?= "1.4E-45".toFloat
  }

  property("Min[Byte Refined Less[0]]") = secure {
    Min[Byte Refined Less[0]].min.value ?= Byte.MinValue
  }

  property("Min[Short Refined Less[0]]") = secure {
    Min[Short Refined Less[0]].min.value ?= Short.MinValue
  }

  property("Min[Int Refined Less[0]]") = secure {
    Min[Int Refined Less[0]].min.value ?= Int.MinValue
  }

  property("Min[Long Refined Less[0]]") = secure {
    Min[Long Refined Less[0]].min.value ?= Long.MinValue
  }

  property("Min[Float Refined Less[0]]") = secure {
    Min[Float Refined Less[0]].min.value ?= Float.MinValue
  }

  property("Min[Double Refined Less[0]]") = secure {
    Min[Double Refined Less[0]].min.value ?= Double.MinValue
  }
  ...
  property("Min[Int Refined Not[Less[-5]]]") = secure {
    Min[Int Refined Not[Less[-5]]].min.value ?= -5
  }

  property("Min[Int Refined Interval.Open[1, 4]]") = secure {
    Min[Int Refined Interval.Open[1, 4]].min.value ?= 2
  }

  property("Min[Double Refined Interval.Open[1, 4]]") = secure {
    Min[Double Refined Interval.Open[1, 4]].min.value ?= 1.0000000000000002
  }

  property("Min[Int Refined Interval.Closed[-20, 10]]") = secure {
    Min[Int Refined Interval.Closed[-20, 10]].min.value ?= -20
  }

  property("Min[Double Refined Interval.Closed[-20.001d, 0d]]") = secure {
    Min[Double Refined Interval.Closed[-20.001d, 0d]].min.value ?= -20.001d
  }

  property("Min[Char Refined Interval.Closed['A', 'Z']]") = secure {
    Min[Char Refined Interval.Closed['A', 'Z']].min.value ?= 'A'
  }
  ...
  property("Min[Int Refined Divisible[5]]") = secure {
    Min[Int Refined Divisible[5]].min.value ?= -2147483645
  }
