...
class BooleanValidateSpec extends Properties("BooleanValidate") {
  ...
  property("AllOf.isValid") = forAll { (i: Int) =>
    isValid[AllOf[(Greater[0], Less[10])]](i) ?= (i > 0 && i < 10)
  }

  property("AllOf.showExpr") = secure {
    showExpr[AllOf[(Greater[0], Less[10])]](5) ?=
      "((5 > 0) && (5 < 10) && true)"
  }

  property("AnyOf.isValid") = forAll { (c: Char) =>
    isValid[AnyOf[(Digit, LowerCase, Whitespace)]](c) ?=
      (c.isDigit || c.isLower || c.isWhitespace)
  }

  property("AnyOf.showExpr") = secure {
    showExpr[AnyOf[(Digit, LowerCase, Whitespace)]]('c') ?=
      "(isDigit('c') || isLower('c') || isWhitespace('c') || false)"
  }

  // `OneOf` has no `Validate` instance on Scala 3 (only its case class is defined), so the
  // Scala 2 `OneOf.*` properties are not ported.
}
