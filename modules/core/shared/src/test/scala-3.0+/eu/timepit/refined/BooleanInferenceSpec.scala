...
class BooleanInferenceSpec extends Properties("BooleanInference") {
  ...
  property("double negation elimination with Greater") = secure {
    // Scala 3 names the numeric rule `greaterInferenceInt` (Scala 2 uses `greaterInference`).
    Inference[Not[Not[Greater[5]]], Greater[4]] ?=
      Inference(5 > 4, "doubleNegationElimination(greaterInferenceInt(5, 4))")
  }
  ...
  property("double negation introduction with Greater") = secure {
    Inference[Greater[5], Not[Not[Greater[4]]]].isValid
  }
  ...
  property("conjunction associativity") = secure {
    Inference[
      (UpperCase And Letter) And Not[Whitespace],
      UpperCase And
        (Letter And
          Not[
            Whitespace
  ...
  property("complex conjunction elimination") = secure {
    ...
    type ConcreteRefinement = And[StartsWith["001"], BaseRefinement]

    Inference[ConcreteRefinement, BaseRefinement].isValid
  }

  property("conjunction introduction") = wellTyped {
    illTyped("Inference[UpperCase, UpperCase And Digit]", "No given instance")
  }
  ...
  property("disjunction elimination") = wellTyped {
    illTyped("Inference[UpperCase Or Digit, Digit]", "No given instance")
  }
